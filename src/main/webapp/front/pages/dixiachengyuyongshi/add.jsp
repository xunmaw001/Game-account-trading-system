	...
	<div class="form-box layui-form" lay-filter="layform">
		<div class="form-control form-content">
			...
			<div>
				...
				<div class="layui-form-item">
					<label class="layui-form-label">游戏跨区</label>
					<div class="layui-input-block">
						<input v-model="detail.youxikuaqu" type="text" name="title" lay-verify="title" autocomplete="off" placeholder="请输入游戏跨区" class="layui-input">
					</div>
				</div>
				<div class="layui-form-item">
				...
				<div class="layui-form-item">
					<label class="layui-form-label">请选择角色职业</label>
					<div class="layui-input-block">
						<select id="jiaosezhiye" name="interest" lay-filter="jiaosezhiye">
							<option :value="item" v-for="(item,index) in jiaosezhiyeOptions">{{item}}</option>
						</select>
					</div>
				</div>
				<div class="layui-form-item">
					<label class="layui-form-label">请选择角色性别</label>
					<div class="layui-input-block">
						<select id="jiaosexingbie" name="interest" lay-filter="jiaosexingbie">
							<option :value="item" v-for="(item,index) in jiaosexingbieOptions">{{item}}</option>
						</select>
					</div>
				</div>
				<div class="layui-form-item">
					<label class="layui-form-label">请选择封号记录</label>
					<div class="layui-input-block">
						<select id="fenghaojilu" name="interest" lay-filter="fenghaojilu">
							<option :value="item" v-for="(item,index) in fenghaojiluOptions">{{item}}</option>
						</select>
					</div>
				</div>
				<div class="layui-form-item">
...
<script type="text/javascript">
	var app = new Vue({
		...
		async created() {
			...
			this.jiaosezhiyeOptions = "男鬼剑,女鬼剑,女枪手,男枪手,格斗家,女格斗,男法师,女法师,暗夜使者,圣职者,女圣职,魔枪士,守护者,枪剑士,外传角色".split(',')
			this.jiaosexingbieOptions = "男,女".split(',')
			this.fenghaojiluOptions = "存在五天封号记录,不存在五天封号记录".split(',')
		...
		methods: {
			...
			if(!isIntNumer(this.detail.zhanghaojiage)){
				...
				addFun("dixiachengyuyongshi", this.detail, (res) => {
					if(res&&res.code==0){
						alert("提交成功")
						window.history.go(-1)
					}
 	...
 	$(function(){
		...
		layui.use(['form','layedit'], function(){
			...
			form.on('select(jiaosezhiye)', function(data){
				app.detail.jiaosezhiye = data.value
			});
			form.on('select(jiaosexingbie)', function(data){
				app.detail.jiaosexingbie = data.value
			});
			form.on('select(fenghaojilu)', function(data){
				app.detail.fenghaojilu = data.value
			});
