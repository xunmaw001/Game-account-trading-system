							...
							<div :class="group?'col-lg-9 col-12':'col-lg-12 col-12'">
								<article>
									<div class="shop-product-wrap grids row justify-content-center"style="justify-content: flex-start!important;">
										<div class="col-lg-4 col-md-6 col-12" v-for="(item, i) in list" @click="onPageTap(`../${item.tablename}/detail.jsp?id=${item.refid})`)">
											<div class="product-item">
													<div class="img-box">
														<img :src="item.picture.split(',')[0]" alt="shope">
													</div>
												<div class="product-content">
													<h6><a :href="`javascript: onPageTap('../${item.tablename}/detail.jsp?id=${item.refid}')`">{{item.name}}</a></h6>
												</div>
											</div>
										</div>
									</div>
							    ...
							    <aside>
							        <div class="widget widget-search">
							            ...
							            <form @submit.prevent="searchnameTap" class="search-wrapper" style="margin-bottom: 20px;">
							                <input v-model="name" type="text" name="s" placeholder="请输入收藏名称">
