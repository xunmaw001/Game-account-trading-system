	...
	<section class="page-header bg_img padding-tb">
		...
		<div class="container">
			<div class="page-header-content-area">
				<h4 class="ph-title">地下城与勇士评论表详情</h4>
				<ul class="agri-ul">
					<li><a href="../../index.jsp">首页</a></li>
					<li><a class="active">地下城与勇士评论表详情</a></li>
				</ul>
			</div>
		</div>
	</section>
						...
						<div class="row">
							<div class="col-md-6 col-12">
								<div class="post-content" v-if="detail">
									<h4>关联表id：{{detail.refid}}</h4>
									<h4>评论内容：{{detail.content}}</h4>
									<h4>用户id：{{detail.userid}}</h4>
...
<script type="text/javascript">
	var app = new Vue({
		...
		async created() {
			...
			if(typeof frontmenus == "object"){
				frontmenus.forEach(item => {
					if(item.menutable == "discussdixiachengyuyongshi"){
		...
		methods: {
			...
			getDetail(id){
				return new Promise((resolve) => {
					goodsInfo('discussdixiachengyuyongshi',id, (res) => {
			...
			getComments(){
				commentList("discussdixiachengyuyongshi", this.detail.id, (res) => {
					if(res&&res.code==0){
			...
			async onCommentTap(){
				...
				if(user){
					...
					addComment("discussdixiachengyuyongshi", data, (res) => {
						if(res&&res.code==0){
							alert("评论成功")
							this.getComments()
						}
