...
<body>
										...
										<div class="col-sm-12">
											<div id="add-row_filter" class="dataTables_filter">
																																																																																																								<label>商品编号 <input type="text" id="shangpinbianhaoSearch" class="form-control form-control-sm" placeholder="" aria-controls="add-row"></label>
																																																																																																										<label>游戏区服 <input type="text" id="youxiqufuSearch" class="form-control form-control-sm" placeholder="" aria-controls="add-row"></label>
																																																																																	<label>游戏跨区 <input type="text" id="youxikuaquSearch" class="form-control form-control-sm" placeholder="" aria-controls="add-row"></label>
																																																																																																										<label>角色职业</label>
																<select name="jiaosezhiye" id="jiaosezhiyeSelect">
																																	<option value=" "> </option>
																																																						<option value="男鬼剑">男鬼剑</option>
																																					<option value="女鬼剑">女鬼剑</option>
																																					<option value="女枪手">女枪手</option>
																																					<option value="男枪手">男枪手</option>
																																					<option value="格斗家">格斗家</option>
																																					<option value="女格斗">女格斗</option>
																																					<option value="男法师">男法师</option>
																																					<option value="女法师">女法师</option>
																																					<option value="暗夜使者">暗夜使者</option>
																																					<option value="圣职者">圣职者</option>
																																					<option value="女圣职">女圣职</option>
																																					<option value="魔枪士">魔枪士</option>
																																					<option value="守护者">守护者</option>
																																					<option value="枪剑士">枪剑士</option>
																																					<option value="外传角色">外传角色</option>
																																																		</select>
											...
											<tr role="row">
													<th style="width: 10px;"><input type="checkbox" onclick="chooseAll()"></th>
																																						<th id="maijiazhanghaoTh" class="sorting" onclick="sort('maijiazhanghao')" tabindex="0" aria-controls="add-row" rowspan="1" colspan="1">卖家账号</th>
																																																			...
																																																			<th id="youxikuaquTh" class="sorting" onclick="sort('youxikuaqu')" tabindex="0" aria-controls="add-row" rowspan="1" colspan="1">游戏跨区</th>
																																																			<th id="jiaosedengjiTh" class="sorting" onclick="sort('jiaosedengji')" tabindex="0" aria-controls="add-row" rowspan="1" colspan="1">角色等级</th>
																																																			<th id="jiaosezhiyeTh" class="sorting" onclick="sort('jiaosezhiye')" tabindex="0" aria-controls="add-row" rowspan="1" colspan="1">角色职业</th>
																																																			<th id="jiaosexingbieTh" class="sorting" onclick="sort('jiaosexingbie')" tabindex="0" aria-controls="add-row" rowspan="1" colspan="1">角色性别</th>
																																																			<th id="fenghaojiluTh" class="sorting" onclick="sort('fenghaojilu')" tabindex="0" aria-controls="add-row" rowspan="1" colspan="1">封号记录</th>
	...
	<script >
		...
		function search(){
			searchForm = { key: ""};
						...
						searchForm.shangpinbianhao ="%" +  $('#shangpinbianhaoSearch').val() + "%";
																																	...
																																	if($('#youxiqufuSearch').val() != null && $('#youxiqufuSearch').val() != ''){
						searchForm.youxiqufu ="%" +  $('#youxiqufuSearch').val() + "%";
					}				
																										if($('#youxikuaquSearch').val() != null && $('#youxikuaquSearch').val() != ''){
						searchForm.youxikuaqu ="%" +  $('#youxikuaquSearch').val() + "%";
					}				
																																	var jiaosezhiyeIndex = document.getElementById("jiaosezhiyeSelect").selectedIndex;
					var jiaosezhiyeOptions = document.getElementById("jiaosezhiyeSelect").options;
					var jiaosezhiyeSelectedOption = jiaosezhiyeOptions[jiaosezhiyeIndex]
					if(jiaosezhiyeSelectedOption.text != null &&  jiaosezhiyeSelectedOption.text != ''){
						searchForm.jiaosezhiye = "%" + jiaosezhiyeSelectedOption.text + "%";
					}		
        ...
        function getDataList() {
			http("dixiachengyuyongshi/page","GET",{
				page: pageIndex,
				limit: pageSize,
				sort: sortColumn,
				order: sortOrder,
																																	...
																																	youxikuaqu : searchForm.youxikuaqu,	
																																										jiaosezhiye : searchForm.jiaosezhiye,	
		...
		function setDataRow(item){
			...
			row.appendChild(checkbox);
            				...
            				var youxikuaquCell  = document.createElement('td');

												 youxikuaquCell.innerHTML = item.youxikuaqu;

                
													row.appendChild(youxikuaquCell);
				
            				var jiaosedengjiCell  = document.createElement('td');

												 jiaosedengjiCell.innerHTML = item.jiaosedengji;

                
													row.appendChild(jiaosedengjiCell);
				
            				var jiaosezhiyeCell  = document.createElement('td');

												  jiaosezhiyeCell.innerHTML = item.jiaosezhiye; 
				  
                
													row.appendChild(jiaosezhiyeCell);
				
            				var jiaosexingbieCell  = document.createElement('td');

												  jiaosexingbieCell.innerHTML = item.jiaosexingbie; 
				  
                
													row.appendChild(jiaosexingbieCell);
				
            				var fenghaojiluCell  = document.createElement('td');

												  fenghaojiluCell.innerHTML = item.fenghaojilu; 
				  
                
													row.appendChild(fenghaojiluCell);
			...
			btnGroup.setAttribute("class","form-button-action");
				...
				crossBtn.setAttribute("onclick","crossTable(" + item.id +",\"dixiachengzhanghaogoumai\")");
				...
				crossBtn.setAttribute("onclick","crossTable(" + item.id +",\"dixiachengsixinliaotian\")");
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("dixiachengyuyongshi/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						alert('删除成功');
						getDataList();		
					}
		...
		function crossTable(id,crossTabName){
			...
			window.sessionStorage.setItem('crossTableName',"dixiachengyuyongshi");
