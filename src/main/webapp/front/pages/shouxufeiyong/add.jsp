			...
			<div>
				<div class="layui-form-item">
					...
					<div class="layui-input-block">
						<input v-model="detail.jiagequjian" type="text" name="title" lay-verify="title" autocomplete="off" placeholder="请输入价格区间" class="layui-input">
					</div>
				</div>
				<div class="layui-form-item">
				...
				<div class="layui-form-item">
					<label class="layui-form-label">请选择平台保障</label>
					<div class="layui-input-block">
						<select id="pingtaibaozhang" name="interest" lay-filter="pingtaibaozhang">
							<option :value="item" v-for="(item,index) in pingtaibaozhangOptions">{{item}}</option>
						</select>
					</div>
				</div>
			</div>
...
<script type="text/javascript">
	var app = new Vue({
		...
		async created() {
			...
			this.pingtaibaozhangOptions = "平台包赔,无保障".split(',')
		},
		methods: {
			onPageTap(url) {
			...
			onConfirmTap(){
			if(!isIntNumer(this.detail.shouxufei)){
				alert(`手续费应输入整数`);
				return
			}
				addFun("shouxufeiyong", this.detail, (res) => {
					if(res&&res.code==0){
						alert("提交成功")
						window.history.go(-1)
					}
 	...
 	$(function(){
		...
		layui.use(['form','layedit'], function(){
			...
			form.on('select(pingtaibaozhang)', function(data){
				app.detail.pingtaibaozhang = data.value
			});
		});
	});
</script>
