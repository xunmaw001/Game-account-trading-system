...
<body>
				...
				<div class="page-inner">
					<div class="page-header">
						<ul class="breadcrumbs">
							...
							<li class="nav-item">
								<a href="${pageContext.request.contextPath}/jsp/modules/dixiachengyuyongshi/list.jsp">地下城与勇士</a>
							</li>
						</ul>
					</div>
					<div class="row">
									...
									<div class="row">
										...
										<form id="addOrUpdateForm" method="post">
											...
											<div class="form-group">
												...
												<input id="youxikuaqu" name="youxikuaqu" class="form-control" placeholder="游戏跨区">
												</div>	
											<div class="form-group">
												<label>角色等级</label>
											...
											<div class="form-group">
												...
												<select id="jiaosezhiyeSelect" name="jiaosezhiye" class="form-control">
															<option value=" "></option>
													 																		<option class="jiaosezhiyeOption" value="男鬼剑">
																男鬼剑
															</option>
																	<option class="jiaosezhiyeOption" value="女鬼剑">
																女鬼剑
															</option>
																	<option class="jiaosezhiyeOption" value="女枪手">
																女枪手
															</option>
																	<option class="jiaosezhiyeOption" value="男枪手">
																男枪手
															</option>
																	<option class="jiaosezhiyeOption" value="格斗家">
																格斗家
															</option>
																	<option class="jiaosezhiyeOption" value="女格斗">
																女格斗
															</option>
																	<option class="jiaosezhiyeOption" value="男法师">
																男法师
															</option>
																	<option class="jiaosezhiyeOption" value="女法师">
																女法师
															</option>
																	<option class="jiaosezhiyeOption" value="暗夜使者">
																暗夜使者
															</option>
																	<option class="jiaosezhiyeOption" value="圣职者">
																圣职者
															</option>
																	<option class="jiaosezhiyeOption" value="女圣职">
																女圣职
															</option>
																	<option class="jiaosezhiyeOption" value="魔枪士">
																魔枪士
															</option>
																	<option class="jiaosezhiyeOption" value="守护者">
																守护者
															</option>
																	<option class="jiaosezhiyeOption" value="枪剑士">
																枪剑士
															</option>
																	<option class="jiaosezhiyeOption" value="外传角色">
																外传角色
															</option>
															</select>
												</div>	
											<div class="form-group">
												...
												<select id="jiaosexingbieSelect" name="jiaosexingbie" class="form-control">
															<option value=" "></option>
													 																		<option class="jiaosexingbieOption" value="男">
																男
															</option>
																	<option class="jiaosexingbieOption" value="女">
																女
															</option>
															</select>
												</div>	
											<div class="form-group">
												...
												<select id="fenghaojiluSelect" name="fenghaojilu" class="form-control">
															<option value=" "></option>
													 																		<option class="fenghaojiluOption" value="存在五天封号记录">
																存在五天封号记录
															</option>
																	<option class="fenghaojiluOption" value="不存在五天封号记录">
																不存在五天封号记录
															</option>
															</select>
												</div>	
	...
	<script>
		...
		var tableName = "dixiachengyuyongshi";
		  ...
		  beforeCreate: function(){
			  var id = window.sessionStorage.getItem("id");
				if(id != null && id != "" && id != "null"){
					$.ajax({ 
		                type: "GET",
		                url: baseUrl + "dixiachengyuyongshi/info/" + id,
		...
		function submit() {
			if(validform() ==true && compare() == true){
				...
				httpJson("dixiachengyuyongshi/"+urlParam,"POST",data,(res)=>{
					if(res.code == 0){
		...
		function getDetails() {
			...
			if(id != null && id != "" && id != "null"){
				...
				updateId = id;
				window.sessionStorage.removeItem('id');
				http("dixiachengyuyongshi/info/" + id,"GET",{},(res)=>{
					if(res.code == 0){
						ruleForm = res.data
						setSelectOption();
						// 设置图片src
		...
		function calculationSE(colName){
			//单列求和接口
			http("dixiachengyuyongshi"+colName,"GET",{
		...
		function dataBind(){
			$("#updateId").val(ruleForm.id);	
												$("#maijiazhanghao").val(ruleForm.maijiazhanghao);	
																...
																$("#youxikuaqu").val(ruleForm.youxikuaqu);	
																$("#jiaosedengji").val(ruleForm.jiaosedengji);	
																var jiaosezhiyeOptions = document.getElementsByClassName("jiaosezhiyeOption");
					for(var jiaosezhiyeCount = 0; jiaosezhiyeCount < jiaosezhiyeOptions.length;jiaosezhiyeCount++){
						if(jiaosezhiyeOptions[jiaosezhiyeCount].getAttribute('value') == ruleForm.jiaosezhiye){
							jiaosezhiyeOptions[jiaosezhiyeCount].setAttribute('selected','selected');
						}
					}
																var jiaosexingbieOptions = document.getElementsByClassName("jiaosexingbieOption");
					for(var jiaosexingbieCount = 0; jiaosexingbieCount < jiaosexingbieOptions.length;jiaosexingbieCount++){
						if(jiaosexingbieOptions[jiaosexingbieCount].getAttribute('value') == ruleForm.jiaosexingbie){
							jiaosexingbieOptions[jiaosexingbieCount].setAttribute('selected','selected');
						}
					}
																var fenghaojiluOptions = document.getElementsByClassName("fenghaojiluOption");
					for(var fenghaojiluCount = 0; fenghaojiluCount < fenghaojiluOptions.length;fenghaojiluCount++){
						if(fenghaojiluOptions[fenghaojiluCount].getAttribute('value') == ruleForm.fenghaojilu){
							fenghaojiluOptions[fenghaojiluCount].setAttribute('selected','selected');
						}
					}
				...
				http(crossTableName + "/info/" + crossTableId,"GET",{},(res)=>{
					if(res.code == 0){
						...
						$('#updateId').val(crossTableId);
												...
												if(res.data != null && res.data != '' && res.data.youxikuaqu != null && res.data.youxikuaqu != ''){

															$("#youxikuaqu").val(res.data.youxikuaqu);
													}
												if(res.data != null && res.data != '' && res.data.jiaosedengji != null && res.data.jiaosedengji != ''){

															$("#jiaosedengji").val(res.data.jiaosedengji);
													}
												if(res.data != null && res.data != '' && res.data.jiaosezhiye != null && res.data.jiaosezhiye != ''){

															var jiaosezhiyeOptions = document.getElementsByClassName("jiaosezhiyeOption");
								for(var jiaosezhiyeCount = 0; jiaosezhiyeCount < jiaosezhiyeOptions.length;jiaosezhiyeCount++){
									if(jiaosezhiyeOptions[jiaosezhiyeCount].getAttribute('value') == res.data.jiaosezhiye){
										jiaosezhiyeOptions[jiaosezhiyeCount].setAttribute('selected','selected');
									}
								}
													}
												if(res.data != null && res.data != '' && res.data.jiaosexingbie != null && res.data.jiaosexingbie != ''){

															var jiaosexingbieOptions = document.getElementsByClassName("jiaosexingbieOption");
								for(var jiaosexingbieCount = 0; jiaosexingbieCount < jiaosexingbieOptions.length;jiaosexingbieCount++){
									if(jiaosexingbieOptions[jiaosexingbieCount].getAttribute('value') == res.data.jiaosexingbie){
										jiaosexingbieOptions[jiaosexingbieCount].setAttribute('selected','selected');
									}
								}
													}
												if(res.data != null && res.data != '' && res.data.fenghaojilu != null && res.data.fenghaojilu != ''){

															var fenghaojiluOptions = document.getElementsByClassName("fenghaojiluOption");
								for(var fenghaojiluCount = 0; fenghaojiluCount < fenghaojiluOptions.length;fenghaojiluCount++){
									if(fenghaojiluOptions[fenghaojiluCount].getAttribute('value') == res.data.fenghaojilu){
										fenghaojiluOptions[fenghaojiluCount].setAttribute('selected','selected');
									}
								}
													}
