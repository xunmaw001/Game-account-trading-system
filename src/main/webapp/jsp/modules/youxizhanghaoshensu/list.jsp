...
<body>
											...
											<tr role="row">
													<th style="width: 10px;"><input type="checkbox" onclick="chooseAll()"></th>
																																						<th id="maijiazhanghaoTh" class="sorting" onclick="sort('maijiazhanghao')" tabindex="0" aria-controls="add-row" rowspan="1" colspan="1">卖家账号</th>
																																																			...
																																																			<th id="goumairiqiTh" class="sorting" onclick="sort('goumairiqi')" tabindex="0" aria-controls="add-row" rowspan="1" colspan="1">购买日期</th>
																																																			
																													<th tabindex="0" aria-controls="add-row" rowspan="1" colspan="1" style="min-width: 100px;">详情截图</th>
																																																																	
																													<th tabindex="0" aria-controls="add-row" rowspan="1" colspan="1" style="min-width: 100px;">申诉说明</th>
																																																																	<th id="shensushijianTh" class="sorting" onclick="sort('shensushijian')" tabindex="0" aria-controls="add-row" rowspan="1" colspan="1">申诉时间</th>
	...
	<script >
		...
		var tableName = "youxizhanghaoshensu";
		var pageType = "list";
	  	var searchForm = { key: ""};
		var pageIndex = 1;
		var pageSize = 10;
        ...
        function getDataList() {
			http("youxizhanghaoshensu/page","GET",{
				page: pageIndex,
				limit: pageSize,
				sort: sortColumn,
				order: sortOrder,
		...
		function setDataRow(item){
			...
			row.appendChild(checkbox);
            				...
            				var xiangqingjietuCell  = document.createElement('td');

								                  var xiangqingjietuImg = document.createElement('img');
				  xiangqingjietuImg.setAttribute('src',item.xiangqingjietu);
				  xiangqingjietuImg.setAttribute('height','100px');
				  xiangqingjietuImg.setAttribute('width','100px');
				  xiangqingjietuCell.appendChild(xiangqingjietuImg);

				
													row.appendChild(xiangqingjietuCell);
				
            				var shensushuomingCell  = document.createElement('td');

												  if(item.shensushuoming != null && item.shensushuoming != "" && item.shensushuoming.length >= 11){
				  	shensushuomingCell.innerHTML = item.shensushuoming.substring(0, 10) + "..."; 
				  }else{
				  	shensushuomingCell.innerHTML = item.shensushuoming;
				  }

				
													row.appendChild(shensushuomingCell);
				
            				var shensushijianCell  = document.createElement('td');

												  shensushijianCell.innerHTML = item.shensushijian;

				
													row.appendChild(shensushijianCell);
			...
			function shDialog(id){
				$("#shId").attr("value",id);
				http("youxizhanghaoshensu/info/" + id,"GET",{},(res)=>{
					if(res.code == 0){
			...
			function sh(){
				...
				httpJson("youxizhanghaoshensu/update","POST",data,(res)=>{
					if(res.code == 0){
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("youxizhanghaoshensu/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						alert('删除成功');
						getDataList();		
					}
		...
		function getSum(colName){
			http("youxizhanghaoshensu" + colName,"GET",{
		...
		function crossTable(id,crossTabName){
			...
			window.sessionStorage.setItem('crossTableName',"youxizhanghaoshensu");
