...
<body>
	...
	<script >
		...
		var tableName = "zhanghaojiaoyijilu";
		var pageType = "list";
	  	var searchForm = { key: ""};
		var pageIndex = 1;
		var pageSize = 10;
        ...
        function getDataList() {
			http("zhanghaojiaoyijilu/page","GET",{
				page: pageIndex,
				limit: pageSize,
				sort: sortColumn,
				order: sortOrder,
		...
		function setDataRow(item){
			...
			btnGroup.setAttribute("class","form-button-action");
				...
				crossBtn.setAttribute("onclick","crossTable(" + item.id +",\"youxizhanghaohuanbang\")");
				...
				crossBtn.setAttribute("onclick","crossTable(" + item.id +",\"youxizhanghaoshensu\")");
			...
			function shDialog(id){
				$("#shId").attr("value",id);
				http("zhanghaojiaoyijilu/info/" + id,"GET",{},(res)=>{
					if(res.code == 0){
			...
			function sh(){
				...
				httpJson("zhanghaojiaoyijilu/update","POST",data,(res)=>{
					if(res.code == 0){
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("zhanghaojiaoyijilu/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						alert('删除成功');
						getDataList();		
					}
		...
		function getSum(colName){
			http("zhanghaojiaoyijilu" + colName,"GET",{
		...
		function crossTable(id,crossTabName){
			...
			window.sessionStorage.setItem('crossTableName',"zhanghaojiaoyijilu");
        ...
        function pay(id){
            window.sessionStorage.setItem('id',id);
			window.sessionStorage.setItem('tableName',"zhanghaojiaoyijilu");
