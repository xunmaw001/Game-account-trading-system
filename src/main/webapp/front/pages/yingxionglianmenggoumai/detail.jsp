	...
	<section class="page-header bg_img padding-tb">
		...
		<div class="container">
			<div class="page-header-content-area">
				<h4 class="ph-title">英雄联盟购买详情</h4>
				<ul class="agri-ul">
					<li><a href="../../index.jsp">首页</a></li>
					<li><a class="active">英雄联盟购买详情</a></li>
				</ul>
			</div>
		</div>
	</section>
						...
						<div class="row">
							<div class="col-md-6 col-12">
								<div class="post-content" v-if="detail">
									...
									<h4>实付金额：{{detail.shifujine}}</h4>
									<h4>用户名：{{detail.yonghuming}}</h4>
									<h4>姓名：{{detail.xingming}}</h4>
									<h4>购买日期：{{detail.goumairiqi}}</h4>
									<h4>是否支付：{{detail.ispay}}</h4>

									<div class="form">
										<button v-for="item in buttons" @click="onCrossTap('yingxionglianmenghuanbang')" v-if="item=='换绑'">换绑</button>
										<button v-for="item in buttons" @click="onCrossTap('yingxionglianmengshensu')" v-if="item=='申诉'">申诉</button>
									</div>
...
<script type="text/javascript">
	var app = new Vue({
		...
		async created() {
			...
			if(typeof frontmenus == "object"){
				frontmenus.forEach(item => {
					if(item.menutable == "yingxionglianmenggoumai"){
		...
		methods: {
			...
			getDetail(id){
				return new Promise((resolve) => {
					goodsInfo('yingxionglianmenggoumai',id, (res) => {
			...
			onCrossTap(tablename){
				...
				window.location.href = `../${tablename}/add.jsp?tablename=yingxionglianmenggoumai`
			},
		}
	})
</script>
