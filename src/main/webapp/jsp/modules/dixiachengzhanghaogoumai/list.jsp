...
<body>
						...
						<div class="card">
							<div class="card-header">
								<div class="d-flex align-items-center">
									<h4 class="card-title">地下城账号购买</h4>
	...
	<script >
		...
		var tableName = "dixiachengzhanghaogoumai";
		var pageType = "list";
	  	var searchForm = { key: ""};
		var pageIndex = 1;
		var pageSize = 10;
        ...
        function getDataList() {
			http("dixiachengzhanghaogoumai/page","GET",{
				page: pageIndex,
				limit: pageSize,
				sort: sortColumn,
				order: sortOrder,
		...
		function setDataRow(item){
			...
			btnGroup.setAttribute("class","form-button-action");
				...
				crossBtn.innerHTML = "换绑"
				crossBtn.setAttribute("type","button")
				crossBtn.setAttribute("class","btn btn-secondary btn-xs");
				crossBtn.setAttribute("data-original-title","换绑");
				crossBtn.setAttribute("onclick","crossTable(" + item.id +",\"dixiachengzhanghaohuanbang\")");
				if(crossBtnControl('换绑') == true){
					btnGroup.appendChild(crossBtn);
				}   
				var crossBtn = document.createElement('button');
				crossBtn.innerHTML = "申诉"
				crossBtn.setAttribute("type","button")
				crossBtn.setAttribute("class","btn btn-secondary btn-xs");
				crossBtn.setAttribute("data-original-title","申诉");
				crossBtn.setAttribute("onclick","crossTable(" + item.id +",\"dixiachengzhanghaoshensu\")");
				if(crossBtnControl('申诉') == true){
					btnGroup.appendChild(crossBtn);
				}   
			...
			function shDialog(id){
				$("#shId").attr("value",id);
				http("dixiachengzhanghaogoumai/info/" + id,"GET",{},(res)=>{
					if(res.code == 0){
			...
			function sh(){
				...
				httpJson("dixiachengzhanghaogoumai/update","POST",data,(res)=>{
					if(res.code == 0){
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("dixiachengzhanghaogoumai/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						alert('删除成功');
						getDataList();		
					}
		...
		function getSum(colName){
			http("dixiachengzhanghaogoumai" + colName,"GET",{
		...
		function crossTable(id,crossTabName){
			...
			window.sessionStorage.setItem('crossTableName',"dixiachengzhanghaogoumai");
        ...
        function pay(id){
            window.sessionStorage.setItem('id',id);
			window.sessionStorage.setItem('tableName',"dixiachengzhanghaogoumai");
