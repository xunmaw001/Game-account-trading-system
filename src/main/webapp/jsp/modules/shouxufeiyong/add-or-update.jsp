				...
				<div class="page-inner">
					<div class="page-header">
						<ul class="breadcrumbs">
							...
							<li class="nav-item">
								<a href="${pageContext.request.contextPath}/jsp/modules/shouxufeiyong/list.jsp">手续费用</a>
							</li>
						</ul>
					</div>
					<div class="row">
									...
									<div class="row">
										...
										<form id="addOrUpdateForm" method="post">
											...
											<div class="form-group">
												...
												<input id="jiagequjian" name="jiagequjian" class="form-control" placeholder="价格区间">
												</div>	
											<div class="form-group">
												<label>手续费</label>
												
												<input id="shouxufei" name="shouxufei" class="form-control" placeholder="手续费">
												</div>	
											<div class="form-group">
												<label>平台保障</label>
												
												<select id="pingtaibaozhangSelect" name="pingtaibaozhang" class="form-control">
															<option value=" "></option>
													 																		<option class="pingtaibaozhangOption" value="平台包赔">
																平台包赔
															</option>
																	<option class="pingtaibaozhangOption" value="无保障">
																无保障
															</option>
															</select>
												</div>	
	...
	<script>
		  ...
		  beforeCreate: function(){
			  var id = window.sessionStorage.getItem("id");
				if(id != null && id != "" && id != "null"){
					$.ajax({ 
		                type: "GET",
		                url: baseUrl + "shouxufeiyong/info/" + id,
		...
		function submit() {
			if(validform() ==true && compare() == true){
				...
				httpJson("shouxufeiyong/"+urlParam,"POST",data,(res)=>{
					if(res.code == 0){
		...
		function getDetails() {
			...
			if(id != null && id != "" && id != "null"){
				...
				updateId = id;
				window.sessionStorage.removeItem('id');
				http("shouxufeiyong/info/" + id,"GET",{},(res)=>{
					if(res.code == 0){
						ruleForm = res.data
						setSelectOption();
						// 设置图片src
		...
		function calculationSE(colName){
			//单列求和接口
			http("shouxufeiyong"+colName,"GET",{
		...
		function dataBind(){
			$("#updateId").val(ruleForm.id);	
												$("#jiagequjian").val(ruleForm.jiagequjian);	
																$("#shouxufei").val(ruleForm.shouxufei);	
																var pingtaibaozhangOptions = document.getElementsByClassName("pingtaibaozhangOption");
					for(var pingtaibaozhangCount = 0; pingtaibaozhangCount < pingtaibaozhangOptions.length;pingtaibaozhangCount++){
						if(pingtaibaozhangOptions[pingtaibaozhangCount].getAttribute('value') == ruleForm.pingtaibaozhang){
							pingtaibaozhangOptions[pingtaibaozhangCount].setAttribute('selected','selected');
						}
					}
									}
		//图片显示
					...
					if(res.code == 0){
						...
						$('#updateId').val(crossTableId);
												if(res.data != null && res.data != '' && res.data.jiagequjian != null && res.data.jiagequjian != ''){
															...
															$("#jiagequjian").val(res.data.jiagequjian);
													}
												...
												if(res.data != null && res.data != '' && res.data.pingtaibaozhang != null && res.data.pingtaibaozhang != ''){
															...
															var pingtaibaozhangOptions = document.getElementsByClassName("pingtaibaozhangOption");
								for(var pingtaibaozhangCount = 0; pingtaibaozhangCount < pingtaibaozhangOptions.length;pingtaibaozhangCount++){
									if(pingtaibaozhangOptions[pingtaibaozhangCount].getAttribute('value') == res.data.pingtaibaozhang){
										pingtaibaozhangOptions[pingtaibaozhangCount].setAttribute('selected','selected');
									}
								}
													}
