...
<body>
				...
				<div class="page-inner">
					<div class="page-header">
						<ul class="breadcrumbs">
							...
							<li class="nav-item">
								<a href="${pageContext.request.contextPath}/jsp/modules/dixiachengzhanghaohuanbang/list.jsp">地下城账号换绑</a>
							</li>
						</ul>
					</div>
					<div class="row">
									...
									<div class="row">
										...
										<form id="addOrUpdateForm" method="post">
											...
											<div class="form-group">
												...
												<input id="dengluzhanghao" name="dengluzhanghao" class="form-control" placeholder="登录账号">
												</div>	
											<div class="form-group">
												<label>登录密码</label>
												
												<input id="denglumima" name="denglumima" class="form-control" placeholder="登录密码">
												</div>	
											<div class="form-group">
												<label>绑定手机</label>
												
												<input id="bangdingshouji" name="bangdingshouji" class="form-control" placeholder="绑定手机">
												</div>	
											<div class="form-group">
												<label>换绑手机</label>
												
												<input id="huanbangshouji" name="huanbangshouji" class="form-control" placeholder="换绑手机">
												</div>	
											<div class="form-group">
												<label>换绑截图</label>
												
													<img id="huanbangjietuImg" src="" width="100" height="100">
													<input name="file" type="file" id="huanbangjietuupload" class="form-control-file">
													<input name="huanbangjietu" id="huanbangjietu-input" type="hidden">
												</div>	
											<div class="form-group">
												<label>换绑时间</label>
												
												<div id="huanbangshijianDate" class="input-append date form_datetime">
													<input id="huanbangshijian-input" name="huanbangshijian" size="20" type="text" readonly>
	...
	<script>
		...
		var tableName = "dixiachengzhanghaohuanbang";
		  ...
		  beforeCreate: function(){
			  var id = window.sessionStorage.getItem("id");
				if(id != null && id != "" && id != "null"){
					$.ajax({ 
		                type: "GET",
		                url: baseUrl + "dixiachengzhanghaohuanbang/info/" + id,
		...
		// 文件上传
		function upload(){
																																																																																																							$('#huanbangjietuupload').fileupload({
						url: baseUrl + 'file/upload',
						headers:{ token: window.sessionStorage.getItem("token")},
						dataType: 'json',
						done: function (e, data) {						
							document.getElementById('huanbangjietu-input').setAttribute('value',baseUrl+"upload/"+data.result.file);
							if(document.getElementById('huanbangjietuFileName') != null){
								document.getElementById('huanbangjietuFileName').innerHTML = data.result.file +"";
							}						
							$("#huanbangjietuImg").attr("src",baseUrl+"upload/"+data.result.file);
						}
					});
																														}  
		// 表单提交
		function submit() {
			if(validform() ==true && compare() == true){
				...
				httpJson("dixiachengzhanghaohuanbang/"+urlParam,"POST",data,(res)=>{
					if(res.code == 0){
		...
		function getDetails() {
			...
			if(id != null && id != "" && id != "null"){
				...
				updateId = id;
				window.sessionStorage.removeItem('id');
				http("dixiachengzhanghaohuanbang/info/" + id,"GET",{},(res)=>{
					if(res.code == 0){
						ruleForm = res.data
						setSelectOption();
						// 设置图片src
		...
		function dateTimePick(){
			$.fn.datetimepicker.dates['zh-CN'] = { 
		            ...
		            meridiem: ["上午", "下午"]
																																																																																																													...
																																																																																																													$("#huanbangshijianDate").datetimepicker({ 
					autoclose: true,
		...
		function calculationSE(colName){
			//单列求和接口
			http("dixiachengzhanghaohuanbang"+colName,"GET",{
		...
		function dataBind(){
			$("#updateId").val(ruleForm.id);	
												$("#maijiazhanghao").val(ruleForm.maijiazhanghao);	
																...
																$("#dengluzhanghao").val(ruleForm.dengluzhanghao);	
																$("#denglumima").val(ruleForm.denglumima);	
																$("#bangdingshouji").val(ruleForm.bangdingshouji);	
																$("#huanbangshouji").val(ruleForm.huanbangshouji);	
																$("#huanbangjietu-input").val(ruleForm.huanbangjietu);
																$("#huanbangshijian-input").val(ruleForm.huanbangshijian);
											
																				$("#shhf").val(ruleForm.shhf);	
									}
		//图片显示
		function showImg(){
																																																																																																							var huanbangjietuFileName = "\"" + ruleForm.huanbangjietu + "\"";
					$("#huanbangjietuImg").attr("src",ruleForm.huanbangjietu);
																														}		
       //跨表
				...
				http(crossTableName + "/info/" + crossTableId,"GET",{},(res)=>{
					if(res.code == 0){
						...
						$('#updateId').val(crossTableId);
												...
												if(res.data != null && res.data != '' && res.data.dengluzhanghao != null && res.data.dengluzhanghao != ''){

															$("#dengluzhanghao").val(res.data.dengluzhanghao);
													}
												if(res.data != null && res.data != '' && res.data.denglumima != null && res.data.denglumima != ''){

															$("#denglumima").val(res.data.denglumima);
													}
												if(res.data != null && res.data != '' && res.data.bangdingshouji != null && res.data.bangdingshouji != ''){

															$("#bangdingshouji").val(res.data.bangdingshouji);
													}
												if(res.data != null && res.data != '' && res.data.huanbangshouji != null && res.data.huanbangshouji != ''){

															$("#huanbangshouji").val(res.data.huanbangshouji);
													}
												if(res.data != null && res.data != '' && res.data.huanbangjietu != null && res.data.huanbangjietu != ''){

															$("#huanbangjietuImg").attr("src",res.data.huanbangjietu);
								$("#huanbangjietu-input").val(res.data.huanbangjietu);
													}
												if(res.data != null && res.data != '' && res.data.huanbangshijian != null && res.data.huanbangshijian != ''){

															$("#huanbangshijian-input").val(res.data.huanbangshijian);
								$('#huanbangshijianFileName').val(res.data.huanbangshijian)
													}
