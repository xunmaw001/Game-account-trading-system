	...
	<section class="page-header bg_img padding-tb">
		...
		<div class="container">
			<div class="page-header-content-area">
				<h4 class="ph-title">英雄联盟详情</h4>
				<ul class="agri-ul">
					<li><a href="../../index.jsp">首页</a></li>
					<li><a class="active">英雄联盟详情</a></li>
				</ul>
			</div>
		</div>
	</section>
						...
						<div class="row">
							...
							<div class="col-md-6 col-12">
								<div class="post-content" v-if="detail">
									...
									<h4>网络大区：{{detail.wangluodaqu}}</h4>
									<h4>游戏区服：{{detail.youxiqufu}}</h4>
									<h4>其他：{{detail.qita}}</h4>
									<h4>限定皮肤：{{detail.xiandingpifu}}</h4>
									<h4>游戏等级：{{detail.youxidengji}}</h4>
									<h4>最高段位：{{detail.zuigaoduanwei}}</h4>
									<h4>当前段位：{{detail.dangqianduanwei}}</h4>
									<h4>英雄数量：{{detail.yingxiongshuliang}}</h4>
									<h4>皮肤数量：{{detail.pifushuliang}}</h4>
									...
									<div class="form">
										<button v-for="item in buttons" @click="onCrossTap('yingxionglianmenggoumai')" v-if="item=='购买'">购买</button>
										<button v-for="item in buttons" @click="onCrossTap('yingxionglianmengsixin')" v-if="item=='私信'">私信</button>
...
<script type="text/javascript">
	var app = new Vue({
		...
		async created() {
			...
			if(typeof frontmenus == "object"){
				frontmenus.forEach(item => {
					if(item.menutable == "yingxionglianmeng"){
		...
		methods: {
			...
			getDetail(id){
				return new Promise((resolve) => {
					goodsInfo('yingxionglianmeng',id, (res) => {
			...
			onCrossTap(tablename){
				...
				window.location.href = `../${tablename}/add.jsp?tablename=yingxionglianmeng`
			},
			async onStoreTap(){
				...
				storeUpAdd("yingxionglianmeng", data, (res) => {
					if(res&&res.code==0){
						alert("收藏成功")
					}
				})
			...
			onThumbsUp(type){
				thumbsUp("yingxionglianmeng", this.detail.id, type, (res) => {
					if(res&&res.code==0){
						if(type==1){
							alert("点赞成功")
			...
			getComments(){
				commentList("yingxionglianmeng", this.detail.id, (res) => {
					if(res&&res.code==0){
			...
			async onCommentTap(){
				...
				if(user){
					...
					addComment("yingxionglianmeng", data, (res) => {
						if(res&&res.code==0){
							alert("评论成功")
							this.getComments()
						}
