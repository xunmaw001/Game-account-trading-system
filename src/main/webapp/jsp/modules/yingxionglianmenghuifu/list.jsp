...
<body>
										...
										<thead>
											...
											<tr role="row">
													<th style="width: 10px;"><input type="checkbox" onclick="chooseAll()"></th>
																													...
																													<th tabindex="0" aria-controls="add-row" rowspan="1" colspan="1" style="min-width: 100px;">用户回复</th>
	...
	<script >
		...
		var tableName = "yingxionglianmenghuifu";
		var pageType = "list";
	  	var searchForm = { key: ""};
		var pageIndex = 1;
		var pageSize = 10;
        ...
        function getDataList() {
			http("yingxionglianmenghuifu/page","GET",{
				page: pageIndex,
				limit: pageSize,
				sort: sortColumn,
				order: sortOrder,
		...
		function setDataRow(item){
			...
			row.appendChild(checkbox);
				  	...
				  	maijiahuifuCell.innerHTML = item.maijiahuifu;
            				...
            				var yonghuhuifuCell  = document.createElement('td');

												  if(item.yonghuhuifu != null && item.yonghuhuifu != "" && item.yonghuhuifu.length >= 11){
				  	yonghuhuifuCell.innerHTML = item.yonghuhuifu.substring(0, 10) + "..."; 
				  }else{
				  	yonghuhuifuCell.innerHTML = item.yonghuhuifu;
				  }

				
													row.appendChild(yonghuhuifuCell);
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("yingxionglianmenghuifu/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						alert('删除成功');
						getDataList();		
					}
		...
		function getSum(colName){
			http("yingxionglianmenghuifu" + colName,"GET",{
				tableName: "yingxionglianmenghuifu",
				columnName: colName
			},(res)=>{
				if(res.code == 0){
					return res.data.sum;
		...
		function crossTable(id,crossTabName){
			...
			window.sessionStorage.setItem('crossTableName',"yingxionglianmenghuifu");
