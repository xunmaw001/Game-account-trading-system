	...
	<section class="page-header bg_img padding-tb">
		...
		<div class="container">
			<div class="page-header-content-area">
				<h4 class="ph-title">游戏账号详情</h4>
				<ul class="agri-ul">
					<li><a href="../../index.jsp">首页</a></li>
					<li><a class="active">游戏账号详情</a></li>
				</ul>
			</div>
		</div>
	</section>
						...
						<div class="row">
							...
							<div class="col-md-6 col-12">
								<div class="post-content" v-if="detail">
									...
									<h4>游戏厂商：{{detail.youxichangshang}}</h4>
									<h4>游戏分类：{{detail.youxifenlei}}</h4>
									...
									<div class="form">
										<button v-for="item in buttons" @click="onCrossTap('zhanghaojiaoyijilu')" v-if="item=='购买'">购买</button>
										<button v-for="item in buttons" @click="onCrossTap('sixinliaotianjilu')" v-if="item=='私信'">私信</button>
...
<script type="text/javascript">
	var app = new Vue({
		...
		async created() {
			...
			if(typeof frontmenus == "object"){
				frontmenus.forEach(item => {
					if(item.menutable == "youxizhanghao"){
		...
		methods: {
			...
			getDetail(id){
				return new Promise((resolve) => {
					goodsInfo('youxizhanghao',id, (res) => {
			...
			onCrossTap(tablename){
				...
				window.location.href = `../${tablename}/add.jsp?tablename=youxizhanghao`
			},
			async onStoreTap(){
				...
				storeUpAdd("youxizhanghao", data, (res) => {
					if(res&&res.code==0){
						alert("收藏成功")
					}
				})
			...
			onThumbsUp(type){
				thumbsUp("youxizhanghao", this.detail.id, type, (res) => {
					if(res&&res.code==0){
						if(type==1){
							alert("点赞成功")
			...
			getComments(){
				commentList("youxizhanghao", this.detail.id, (res) => {
					if(res&&res.code==0){
			...
			async onCommentTap(){
				...
				if(user){
					...
					addComment("youxizhanghao", data, (res) => {
						if(res&&res.code==0){
							alert("评论成功")
							this.getComments()
						}
