...
<body>
										...
										<thead>
											...
											<tr role="row">
													<th style="width: 10px;"><input type="checkbox" onclick="chooseAll()"></th>
																													...
																													<th tabindex="0" aria-controls="add-row" rowspan="1" colspan="1" style="min-width: 100px;">私信内容</th>
																																																																	<th id="sixinshijianTh" class="sorting" onclick="sort('sixinshijian')" tabindex="0" aria-controls="add-row" rowspan="1" colspan="1">私信时间</th>
	...
	<script >
		...
		var tableName = "sixinliaotianjilu";
		var pageType = "list";
	  	var searchForm = { key: ""};
		var pageIndex = 1;
		var pageSize = 10;
        ...
        function getDataList() {
			http("sixinliaotianjilu/page","GET",{
				page: pageIndex,
				limit: pageSize,
				sort: sortColumn,
				order: sortOrder,
		...
		function setDataRow(item){
			...
			row.appendChild(checkbox);
            				...
            				var sixinneirongCell  = document.createElement('td');

												  if(item.sixinneirong != null && item.sixinneirong != "" && item.sixinneirong.length >= 11){
				  	sixinneirongCell.innerHTML = item.sixinneirong.substring(0, 10) + "..."; 
				  }else{
				  	sixinneirongCell.innerHTML = item.sixinneirong;
				  }

				
													row.appendChild(sixinneirongCell);
				
            				var sixinshijianCell  = document.createElement('td');

												  sixinshijianCell.innerHTML = item.sixinshijian;

				
													row.appendChild(sixinshijianCell);
			...
			btnGroup.setAttribute("class","form-button-action");
				...
				crossBtn.innerHTML = "回复"
				crossBtn.setAttribute("type","button")
				crossBtn.setAttribute("class","btn btn-secondary btn-xs");
				crossBtn.setAttribute("data-original-title","回复");
				crossBtn.setAttribute("onclick","crossTable(" + item.id +",\"sixinliaotianhuifu\")");
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("sixinliaotianjilu/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						alert('删除成功');
						getDataList();		
					}
		...
		function getSum(colName){
			http("sixinliaotianjilu" + colName,"GET",{
		...
		function crossTable(id,crossTabName){
			...
			window.sessionStorage.setItem('crossTableName',"sixinliaotianjilu");
