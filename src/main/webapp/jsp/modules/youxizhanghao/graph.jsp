...
<body>
	...
	<script >
		...
		$.ajax({ 
			type: "GET",
			url: baseUrl+"value/youxizhanghao/"+xColumnName+'/'+yColumnName,
			data:{ },
		...
		function group(colName){
			$.ajax({ 
                type: "GET",
                url: baseUrl+"group/youxizhanghao/"+colName,
             	data:{	tableName: "youxizhanghao",
                    	columnName: colName
					 },
                beforeSend: function(xhr) {
                    xhr.setRequestHeader("token", window.sessionStorage.getItem('token'));
